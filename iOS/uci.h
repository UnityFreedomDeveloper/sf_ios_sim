...
void set_new_position(Position& pos, StateListPtr& states, std::deque<Move> &moveHistory);
void set_custom_position(Position& pos, StateListPtr& states, std::deque<Move> &moveHistory, std::string fen);
void pos_possible_moves(const Position &pos, int &size, int *mPointer);
int pos_evaluation(const Position& _pos);
...
//call from swift
void normal_move(int from, int to, Position &pos, std::deque<Move> &moveHistory);
